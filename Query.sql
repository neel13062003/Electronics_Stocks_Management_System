...
//Lets Create Cursor in our program
...
Insert into Orders(OID, Item_name, amount)  values (13, 'Tv',10000);
...
/*New Trigger & Functions for those supplier purchase 0 items - throw trgger*/

*Trigger*
CREATE TRIGGER checkage11
...
CREATE OR REPLACE FUNCTION fun() RETURNS TRIGGER AS $$                                                     
    BEGIN
        IF new.quantity < 0 THEN
            RAISE EXCEPTION 'You Must Order Something!!!';
            /*RETURN NEW;*/
        END IF;   
        RETURN New;
    END;
...
Insert into Purchase_order(Purchase_id,S_id,pur_product,quantity,amount)   values ('P5',1020304050,'Phone',10,50000);
